...
const actions = {
  async getUserWorkspaces(context){
    let indexFile = context.rootState.profile.profile.storage+"public/popock/workspaces.ttl"
    let workspacesDoc = {}
    let workspaces = []
    try{
  ...
  async addWorkspace(context, workspace) {
    ...
    workspace.path = context.rootState.profile.profile.storage+"public/"+ttl_name+"/"
    let workspacesDoc = {}
    try{
      workspacesDoc = await fetchDocument(indexFile)
    }catch(e){
